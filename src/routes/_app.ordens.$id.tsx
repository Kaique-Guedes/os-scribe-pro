...
function OsDetail() {
  ...
  // Mapeia "etapa concluída" -> próximo status da O.S., e a lista de status a
  // PARTIR de onde é seguro avançar (nunca sobrescreve um status que já passou
  // dessa fase, nem "cancelada"/"atrasada" — mesma trava usada em
  // conferencia-material.functions.ts pro fluxo de chegada de material).
  const AVANCO_STATUS_POR_ETAPA: Partial<
    Record<EtapaTipo, { proximo: OsStatus; apartirDe: OsStatus[] }>
  > = {
    solicitacao_material: { proximo: "aguardando_material", apartirDe: ["aberta"] },
    chegada_material: { proximo: "em_producao", apartirDe: ["aberta", "aguardando_material"] },
    pintura: { proximo: "em_pintura", apartirDe: ["aberta", "aguardando_material", "em_producao"] },
  };
  ...
  const updateEtapa = useMutation({
    mutationFn: async ({
      ...
      // Etapa "entrega" concluída: preenche data_entrega_real e muda status pra "entregue".
      // Guard exclui só "entregue"/"cancelada" (permite vir de qualquer outro status,
      ...
      // Demais etapas: avança o status conforme o mapeamento, só se ainda estiver
      // num status anterior ao alvo (nunca regride nem pula por cima de algo manual).
      // Só roda pra quem NÃO é só-almoxarifado: esse papel não tem permissão de
      // UPDATE em ordens_servico (por design, ver comentário em
      // conferencia-material.functions.ts) — pra ele, o avanço de status de
      // chegada_material já acontece pelo fluxo de conferência, com client admin.
      const avanco = AVANCO_STATUS_POR_ETAPA[tipo];
      if (avanco && status === "concluido" && !restrito) {
        const { error: osErr } = await supabase
          .from("ordens_servico")
          .update({ status: avanco.proximo })
          .eq("id", id)
          .in("status", avanco.apartirDe);
        if (osErr) throw osErr;
      }
    },
    onSuccess: () => {
